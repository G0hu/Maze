...
public class MazeTest extends TestCase {
    ...
    private final static int[][] LABYRINTH = { { 1, 1, 1, 3, 1 }, { 1, 0, 0, 0, 1 }, { 1, 2, 1, 1, 1 } };
	...
	assertTrue("Initial maze should NOT have animals in it", maze.getAnimals().size() == 0);

	// adds dummy animal
	Animal dummy = new Mouse(new Vector2D(3, 0));
	maze.addAnimal(dummy);

	assertTrue("Maze Animals should contain one Animal", maze.getAnimals().size() == 1);

	// retrieves dummy anima from Maze
	Animal retrieved = maze.getAnimals().get(0);

	assertTrue("Maze should contain the Mouse, even at the exit", maze.hasAnimal(dummy));
	assertTrue("Mouse inside the Maze should be the same as Mouse added", retrieved == dummy);
	assertFalse("Maze with one Mouse even at the exit should NOT be solved", maze.isSolved());

	// removes dummy animal
	maze.removeAnimal(dummy);

	assertFalse("Maze should NOT contain the Mouse anymore", maze.hasAnimal(dummy));
	assertTrue("Maze should NOT have anymore Animal in it", maze.getAnimals().size() == 0);
